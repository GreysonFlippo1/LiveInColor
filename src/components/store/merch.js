module.exports = {
    id: 0,
    kind: 'merch',
    items: [
        {
            id: 0,
            snippet: {
                title: 'T-Shirts',
                description: 'This comfortable LiveInColor shirt looks good, and helps a charitable cause! Buy one and we will donate all profits to the Boys and Girls Club! Includes free stickers and wristbands!',
                thumbnails: {
                    medium: {
                        url:'http://www.liveincolormedia.com/Merch_Images/Shirts/singleshirtTHUMB.png'
                    }
                },
                images: [
                    'http://www.liveincolormedia.com/Merch_Images/Shirts/singleshirtTHUMB.png',
                    'http://www.liveincolormedia.com/Merch_Images/Shirts/shirtaldaySLIDE.png',
                    'http://www.liveincolormedia.com/Merch_Images/Shirts/shirtadSLIDE.png'
                    ],
                sizes_available: [0,0,0,0,0],
                stock: 0,
                price: 20.00,
            }
        },
        {
            id: 1,
            snippet: {
                title: 'Masks',
                description: 'COMING SOON! The protective and stylish masks will have you turning heads while staying safe. Buy one and we will donate all profits to www.bravegowns.com!',
                thumbnails: {
                    medium: {
                        url:'http://www.liveincolormedia.com/Merch_Images/Masks/maskJohnny.jpeg'
                    }
                },
                images: [
                    'http://www.liveincolormedia.com/Merch_Images/Masks/maskJohnny.jpeg',
                    'http://www.liveincolormedia.com/Merch_Images/Masks/mask2.jpg',
                    'http://www.liveincolormedia.com/Merch_Images/Masks/mask3.jpg'
                ],
                sizes_available: [0,0,0,0,0],
                stock: 50,
                price: 10.00,
            }
        },
        {
            id: 3,
            snippet: {
                title: 'Shoes',
                description: 'Made from hydro-dipping, these truly one-of-a-kind kicks will make you the most interesting person every time you step out the door. We can do custom orders, please email us with your request. Limited sizes available.',
                thumbnails: {
                    medium: {
                        url:'http://www.liveincolormedia.com/Merch_Images/Shoes/shoes1.jpg'
                    }
                },
                images: [
                    'http://www.liveincolormedia.com/Merch_Images/Shoes/shoes1.jpg',
                    'http://www.liveincolormedia.com/Merch_Images/Shoes/shoes2.jpg'
                    ],
                sizes_available: [0,0,0,0,0],
                stock: 0,
                price: 35.00,
            }
        },
        {
            id: 4,
            snippet: {
                title: 'Wristbands',
                description: 'The colorful wristbands add the perfect amount of style to any fit you own. Get 5 different color combinations for the ultimate mix and match! All profits will go to the Boys and Girls Club!',
                thumbnails: {
                    medium: {
                        url:'http://liveincolormedia.com/Merch_Images/Wristbands/wristbands.JPG'
                    }
                },
                images: [
                    'http://liveincolormedia.com/Merch_Images/Wristbands/wristbands.JPG',
                    ],
                sizes_available: [0,0,0,0,0],
                stock: 300,
                price: 5.00,
            }
        },
    ]
}
